...
public class GetPinnedAndTrendingChannelsAPITest{
	/*
	 * String baseUrl; String apiUrl; String url; int statusCode; Response response;
	 * 
	 * public GetPinnedAndTrendingChannelsAPITest() throws IOException { super(); }
	 * 
	 * @BeforeMethod public void setUp() { baseUrl = prop.getProperty("URL"); apiUrl
	 * = prop.getProperty("ApiURL"); url = baseUrl + apiUrl; }
	 * 
	 * @Test public void getPinnedAndTrendingChannelsTest() throws IOException { //
	 * LoginAPITest login = new LoginAPITest(); // login.loginTest("iphone",
	 * "iphone@123"); System.out.
	 * println("Test Case1: Verify that API returns all the Pinned Channels and Trending Channels"
	 * );
	 ...
	 * body(Payload.getPinnedAndTrendingChannels()). when(). post(url).
	 * then().extract().response(); System.out.println(response.asString());
	 * log.info("Get all pinned and trending channels request execution started"); }
	 * 
	 * @Test public void verifyStatusCodeForTrendingandPinnedChannels() { log.info(
