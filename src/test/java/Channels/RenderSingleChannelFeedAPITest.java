...
public class RenderSingleChannelFeedAPITest extends TestBase {
	...
	public RenderSingleChannelFeedAPITest() throws IOException {
		super();
	}
	...
	@Test
	public void renderSingleChannelFeedTest() throws IOException {
	//	 LoginAPITest login = new LoginAPITest();
	//	 login.loginTest("iphone", "iphone@123");
		Login.getLoginResponse();
		System.out.println("Test Case1: Verify that API returns all the posts of a particular channel.");
		log.info("Render Single Channel feed request execution started");
		response = given().
				header("Content-Type", "application/json").
				header("User-Agent", "Greenorbit").
				header("Authorization", Login.accessToken).
				body(Payload.renderSingleChannelFeed()).
				when().
				post(url).
